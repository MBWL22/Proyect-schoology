$(window).scroll(function(){
    ...
    if ($(this).scrollTop() > 725){
         $('#div-scroll').addClass("fixed bg-black");
         $('.contenedor').addClass("margen");
    }
    else {
        $('#div-scroll').removeClass("fixed bg-black");
...
$("#style-b").click(function(){
    ...
    if (lgpassword==true || lgemail==true){
        ...
        $.ajax({
            url:"ajax/login.php",
            data:"emailUsuario="+$("#user-email-login").val()+"&password="+$("#password-login").val(),
            method:"POST",
            dataType:"json",
            success:function(respuesta){
                ...
                window.location = "home-instructor.php"; //Redirreccionar desde js
                    ...
                    window.location = "home-estudiante.php"; //Redirreccionar desde js
                }else{
                    console.log(respuesta);
                    $("#div-error-login").html('<div class="error-div"><i class="fas fa-exclamation-circle" style="color: #E3556A; font-size: 25px">'+
